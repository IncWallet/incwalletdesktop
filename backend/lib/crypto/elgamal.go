...
func (ciphertext *elGamalCipherText) SetBytes(bytes []byte) error {
	if len(bytes) == 0 {
		return errors.New("cannot setbyte elgamal")
	}
...
func GenerateSerialNumber(privateKey, snd []byte) []byte {
	sndScalar := new(Scalar).FromBytesS(snd)
	privakeyScalar := new(Scalar).FromBytesS(privateKey)
	snPoint := new(Point).Derive(PedCom.G[PedersenPrivateKeyIndex],
