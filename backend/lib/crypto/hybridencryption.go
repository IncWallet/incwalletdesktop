...
import (
	"encoding/json"
	"errors"
	cm "github.com/incognitochain/incognito-chain/common"
...
func (ciphertext *HybridCipherText) SetBytes(bytes []byte) error {
	if len(bytes) == 0 {
		return errors.New("cannot setbyte hybric encryption")
	}

	if len(bytes) < common.ElGamalCiphertextSize {
		// out of range
		return errors.New("out of range Parse ciphertext")
	}
	ciphertext.symKeyEncrypted = bytes[0:common.ElGamalCiphertextSize]
	ciphertext.msgEncrypted = bytes[common.ElGamalCiphertextSize:]
	return nil
}
